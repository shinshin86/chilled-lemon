...
function getPngInfoJson(infoString: string): PngInfoObject {
      ...
      while ((match = re.exec(line)) !== null) {
        ...
        if (key === "Lora hashes") {
          ...
          while (controlNetMatch !== null) {
            ...
            if (
              ["Weight", "Processor Res", "Guidance Start", "Guidance End"]
                .includes(controlNetKey)
            ) {
              controlNetObj[convertToCamelCase(controlNetKey)] = Number(
                controlNetValue,
              );
              ...
              controlNetObj[convertToCamelCase(controlNetKey)] =
                controlNetValue;
            }
  ...
  data[convertToCamelCase("Prompt")] = prompt.trim().split(",").map((item) =>
    item.trim()
  ).filter((item) => item);
  data[convertToCamelCase("Negative prompt")] = negativePrompt.trim().split(",")
      ...
      for (const controlNetObj of controlNetObjList) {
        ...
        const controlNetValueList = Object.keys(controlNetObj).map((key) => {
          ...
          if (["lowVram", "pixelPerfect"].includes(key)) {
            controlNetValue =
              controlNetObj[key].toString().charAt(0).toUpperCase() +
              controlNetObj[key].toString().slice(1);
          }
